...
class TrackFeatures:
    ...
    def max_int_ch2(self):
        """Extract maximum intensity (a.u. fluorescence) of third channel."""
        for i,track in enumerate(self.tracks):
            
            if return_track_attributes.return_track_category(self.tracks,i) in self.track_categories:
                
                self.feature_matrix.append(np.max(return_track_attributes.return_track_amplitude_one_channel(self.tracks, i, 2)))
    ...
    def max_consecutive_significant_ch2(self):
        """Extract the maximum number (counts) of consecutive significant detections below the designated p-value in the third channel."""        
        for i,track in enumerate(self.tracks):
